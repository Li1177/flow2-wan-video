...
class WanVideoEnhancer_F2:
    ...
    FUNCTION = "process"
    CATEGORY = "Custom/Enhancer"  # 更改为唯一分类，避免冲突

    def process(self, images, upscale_model, interpolate_model, upscale_factor, interpolate_frame, order):
        def upscale(images):
    ...
    def interpolate(self, model_name, images, framerate):
        ...
        try:
            ...
            raise ImportError("请确保 frame_interpolation 模块已正确安装在 flow2-wan-video 目录下")

        # 下载插帧模型
        model_path = folder_paths.get_full_path("vfi_models", model_name)
        if not model_path:
            raise FileNotFoundError(f"插帧模型 {model_name} 未找到，请确保模型文件在 ComfyUI/models/vfi_models/ 目录下")

        model = IFNet(arch_ver="4.7")
        sd = torch.load(model_path, map_location="cpu")
        model.load_state_dict(sd)
        del sd
        model.eval().to(mm.get_torch_device())
    ...
    def upscale(self, model_name, images, factor):
        ...
        mm.free_memory(memory_required, mm.get_torch_device())

        model.to(mm.get_torch_device())
        in_img = images.movedim(-1, -3).to(mm.get_torch_device())
